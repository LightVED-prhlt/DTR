...
from helpers import complement_idx, complement_idx_2
...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            # print("before left tokens:", left_tokens, end=", ")
            left_tokens = math.ceil(self.keep_rate * (N - 1))
            # print("after left tokens:", left_tokens)
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        if index is not None:
            ...
            # compute complement indices and the embeddings that were NOT selected (non-topk)
            compl = complement_idx(idx, N - 1)  # [B, N-1-left_tokens]
            non_topk = torch.gather(non_cls, dim=1, index=compl.unsqueeze(-1).expand(-1, -1, C))  # [B, N-1-left_tokens, C]
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        n_tokens = x.shape[1] - 2 if self.fuse_token else x.shape[1] - 1

        # Always return `idx` (may be None) so caller can reconstruct original order.
        return x, n_tokens, idx, non_topk
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', keep_rate=(1, ), fuse_token=False, **kwargs):
        """
        Args:
        ...
        self.keep_rate = keep_rate
        self.depth = depth
        self.with_dtr = kwargs.get('with_dtr', False)
    ...
    def forward_features(self, x, keep_rate=None, tokens=None, get_idx=False):
        ...
        for i, blk in enumerate(self.blocks):
            x, left_token, idx, _ = blk(x, keep_rate[i], tokens[i], get_idx)
    ...
    def forward_features_2(self, x, keep_rate=None, tokens=None, get_idx=False):
        _, _, h, w = x.shape
        if not isinstance(keep_rate, (tuple, list)):
            keep_rate = (keep_rate, ) * self.depth
        if not isinstance(tokens, (tuple, list)):
        ...
        prev_non_topk = None
        prev_idx = None
        prev_left_token = None
        prev_fuse = False
        for i, blk in enumerate(self.blocks):
            # If the previous block returned embeddings for non-topk tokens, reinsert them
            # into their original positions (reconstruct full token order) instead of concatenating.
            if prev_non_topk is not None and self.keep_rate[i] < 1.:
                # number of tokens that were kept by previous block
                kept = prev_left_token
                # total non-class tokens at that stage
                total_non = kept + prev_non_topk.shape[1]

                # extract topk embeddings from current x (current x holds previous block output)
                if prev_fuse:
                    topk_embeddings = x[:, 1:-1]
                else:
                    topk_embeddings = x[:, 1:1 + kept]

                B, _, C = topk_embeddings.shape
                recon_non = torch.zeros(B, total_non, C, device=x.device, dtype=x.dtype)

                if prev_idx is not None:
                    recon_non.scatter_(1, prev_idx.unsqueeze(-1).expand(-1, -1, C), topk_embeddings)
                    compl = complement_idx(prev_idx, total_non)
                    recon_non.scatter_(1, compl.unsqueeze(-1).expand(-1, -1, C), prev_non_topk)
                else:
                    # fallback: append if indices missing
                    recon_non = torch.cat([topk_embeddings, prev_non_topk], dim=1)

                x = torch.cat([x[:, :1], recon_non], dim=1)

            x, left_token, idx, temp_non_topk = blk(x, keep_rate[i], tokens[i], get_idx)
            left_tokens.append(left_token)
            if idx is not None:
                idxs.append(idx)

            # store for next iteration reinsertion; only update when non-topk embeddings are returned
            if temp_non_topk is not None:
                prev_non_topk = temp_non_topk
                prev_idx = idx
                prev_left_token = left_token
                prev_fuse = getattr(blk, 'fuse_token', False)
    ...
    def forward(self, x, keep_rate=None, tokens=None, get_idx=False):
        if self.with_dtr:
            x, _, idxs = self.forward_features_2(x, keep_rate, tokens, get_idx)
        else:
...
def deit_small_patch16_shrink_base(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    if kwargs.get('with_dtr', False) == False:
        keep_rate = [1] * 12
        for i, loc in enumerate(drop_loc):
            keep_rate[loc] = base_keep_rate
        keep_rate = [1, 1, 1, base_keep_rate, 1, 1, base_keep_rate, 1, 1, base_keep_rate, 1, 1]
    else:
        if base_keep_rate == 0.7:
            keep_rate = [1, 1, 1, base_keep_rate, 1, 1, 0.495, 1, 1, 0.355, 1, 1] # 0.7 base_keep_rate
        elif base_keep_rate == 0.6:
            keep_rate = [1, 1, 1, base_keep_rate, 1, 1, 0.36, 1, 1, 0.22, 1, 1] # 0.6 base_keep_rate
        elif base_keep_rate == 0.5:
            keep_rate= [1, 1, 1, base_keep_rate, 1, 1, 0.255, 1, 1, 0.13, 1, 1] # 0.5 base_keep_rate
