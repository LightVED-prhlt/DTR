...
from timm.utils import NativeScaler, get_state_dict, ModelEma, NativeScalerWithUpdateGrad
...
import models
import utils
from helpers import speed_test, get_macs, adjust_keep_rate
...
def get_args_parser():
    ...
    parser.add_argument('--with_dtr', action='store_true', help='whether to use dynamic token refinement (DTR) for evaluation')
...
def main(args):
    ...
    # loss_scaler = NativeScalerWithUpdateGrad()
    loss_scaler = NativeScalerWithUpdateGrad()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % test_interval == 0 or epoch == args.epochs - 1:
            ...
            print(f"Accuracy of the network on the {len(dataset_val)} test images with keep rate {keep_rate:.3f}: {test_stats['acc1']:.3f}%")
            # determine if this is the best checkpoint before updating max_accuracy
            is_best = test_stats["acc1"] > max_accuracy
            if is_best:
                keep_rate_best = keep_rate
                max_accuracy = test_stats["acc1"]
            print(f'Max accuracy: {max_accuracy:.3f}% | Keep rate at max accuracy: {keep_rate_best:.3f}')
            test_stats1 = {f'test_{k}': v for k, v in test_stats.items()}
        else:
            test_stats1 = {}

        # Always save the latest checkpoint; if this epoch produced the best
        # test accuracy, also save a copy as the best checkpoint.
        if args.output_dir:
            checkpoint = {
