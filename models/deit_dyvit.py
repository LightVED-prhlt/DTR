...
class VisionTransformerDiffPruning(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #     prune_ratio_or_n_tokens=None,
        #     revive_ratio_or_n_tokens=1.0
        # )
            ...
            if i in self.pruning_loc:
                if self.training:
                    ...
                    if dead_tokens is not None:
                        x = torch.cat([x, dead_tokens], dim=1)
                    spatial_x = x[:, 1:]
                    pred_score = self.score_predictor[p_count](spatial_x, prev_decision).reshape(B, -1, 2)
                    score = pred_score[:,:,0]
                    num_keep_node = int(init_n * self.token_ratio[p_count])
                    keep_policy = torch.argsort(score, dim=1, descending=True)[:, :num_keep_node]
                    dead_tokens = torch.argsort(score, dim=1, descending=True)[:, num_keep_node:]
